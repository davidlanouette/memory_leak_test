...
namespace Test
{
	public class AsyncTest1 : TestServer
	{
		private ListenerService1 listener;
			
		public void Run(string url = "http://localhost:5001")
		{
			listener = new ListenerService1(new Uri(url));
	...
	class ListenerService1 : IDisposable
	{
		...
		public ListenerService1(Uri listenerPrefix)
		{
			Uri = listenerPrefix;
		...
		void StartWaitingForConnections ()
		{
			while (Listening)
			{
				try
				{
					...
					// create a new thread to handle every incomming request.
					Thread t = new Thread(HandleIncomingRequest);
					t.Start(request);
				}
				catch (Exception e) { 
